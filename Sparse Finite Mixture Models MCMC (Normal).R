...
#Data y[i]
y = rep(NA, N.sum)
y[1:200] = rnorm(n = 200, mean = 10)
y[201:400] = rnorm(n = 200, mean = 30)
y[401:600] = rnorm(n = 200, mean = 50)
y[601:800] = rnorm(n = 200, mean = 70)
y[801:1000] = rnorm(n = 200, mean = 90)
...
for(i in 2:niter) {  
  ...
  for (k in 1:K) {
    theta[i, k] = ifelse(N[i - 1, k] != 0, 
                         rnorm(n = 1, mean = (sum((S[i - 1, ] == k)) * tau0^2 * mean(y)) / (sum((S[i - 1, ] == k)) * tau0^2 + 1), 
                               sd = sqrt(tau0^2 / (sum((S[i - 1, ] == k)) * tau0^2 + 1))),
                         rnorm(n = 1, mean = 0, sd = 100))
  }
  #b
  for (k in 1:(K - 1)) {
  ...
  for (n in 1:N.sum) {
    ...
    for (k in 1:K) {
      ...
      #pr[k] = eta[i, k] * exp(- (y[n] - theta[i, k])^2 / 2)
      pr[k] = exp(log(eta[i, k]) * (- (y[n] - theta[i, k])^2 / 2)/1000) #devided by 1000 so that the exponential won't go to infinity
    }
